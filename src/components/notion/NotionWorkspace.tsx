...
import {
  ...
  FileText, Columns, Rows, Tag, Hash, Calendar, CheckSquare,
  Sparkles, Save, Search, Download, FileSpreadsheet, BookOpen,
  Heading1, Heading2, Quote, CheckCircle2, RefreshCw
} from 'lucide-react';
import { db } from '../../lib/firebaseDb';
import { User } from '../../lib/types';
import { showToast } from '../common/Toast';

export type ColumnType = 'text' | 'number' | 'select' | 'status' | 'date' | 'checkbox';
...
export const NotionWorkspace: React.FC<NotionWorkspaceProps> = ({
  ...
  const [columns, setColumns] = useState<NotionColumn[]>(() => {
    ...
    return [
      ...
      { id: 'col-status', name: 'Status', type: 'status', options: ['Not Started', 'In Progress', 'In Review', 'Completed'] },
      { id: 'col-owner', name: 'Assignee', type: 'text' },
      { id: 'col-priority', name: 'Priority', type: 'select', options: ['P0 - Critical', 'P1 - High', 'P2 - Normal'] },
  ...
  const handleAddColumn = (e: React.FormEvent) => {
    ...
    const newCol: NotionColumn = {
      ...
      type: newColType,
      options: newColType === 'select' || newColType === 'status' ? ['Option 1', 'Option 2', 'Option 3'] : undefined,
    };

    setColumns([...columns, newCol]);
    setNewColName('');
    setShowAddCol(false);
    showToast('Column Created', `Added ${newCol.name} (${newCol.type}) to database.`);
  };
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'Completed': return 'bg-emerald-950/40 border-emerald-800/50 text-emerald-400';
      case 'In Progress': return 'bg-indigo-950/40 border-indigo-800/50 text-indigo-300';
      case 'In Review': return 'bg-amber-950/40 border-amber-800/50 text-amber-300';
      default: return 'bg-[#181820] border-[#242430] text-[#707080]';
    }
  };
  ...
  return (
    <div className="hesics-card p-6 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-[#1A1A22] pb-4">
        <div>
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h2 className="text-sm font-bold text-[#F4F4F6] font-display">{scopeTitle}</h2>
              <p className="text-xs text-[#808090]">Dynamic Notion database & collaborative document space.</p>
            </div>
          </div>
        </div>
          ...
          <div className="flex items-center bg-[#09090D] border border-[#1C1C26] rounded-xl p-1">
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${
                activeSubTab === 'table' ? 'bg-[#77727E] text-white shadow' : 'text-[#707080] hover:text-white'
              }`}
            >
            ...
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${
                activeSubTab === 'notes' ? 'bg-[#77727E] text-white shadow' : 'text-[#707080] hover:text-white'
              }`}
            >
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <span className="text-xs font-mono text-[#707080]">
              Showing <span className="text-[#F4F4F6] font-semibold">{rows.length}</span> entries · <span className="text-[#F4F4F6] font-semibold">{columns.length}</span> properties
            </span>
            ...
            <table className="w-full text-left text-xs border-collapse">
              <thead>
                <tr className="bg-[#0D0D12] border-b border-[#1A1A24] text-[#808090]">
                  <th className="p-3 w-10 text-center font-mono text-[10px] text-[#505060]">#</th>
                  {columns.map((col) => (
                    <th key={col.id} className="p-3 font-semibold text-[#D4D4D8] border-r border-[#171720] min-w-[160px]">
                      <div className="flex items-center justify-between group">
                        <span className="truncate">{col.name}</span>
                        <span className="text-[9px] font-mono text-[#505060] uppercase">{col.type}</span>
                      </div>
                    </th>
                  ))}
              ...
              <tbody className="divide-y divide-[#14141C]">
                {rows.length === 0 ? (
                  <tr>
                    <td colSpan={columns.length + 2} className="p-8 text-center text-xs text-[#505060]">
                      Empty database. Click "New Row" to create entries.
                    </td>
                  </tr>
                ) : (
                  rows.map((row, rIdx) => (
                    <tr key={row.id} className="hover:bg-[#111118] transition-colors">
                      <td className="p-3 text-center font-mono text-[10px] text-[#505060]">{rIdx + 1}</td>
                      {columns.map((col) => {
                        const val = row.cells[col.id];

                        if (col.type === 'checkbox') {
                          return (
                            <td key={col.id} className="p-3 border-r border-[#171720]">
                              <input
                                type="checkbox"
                                checked={!!val}
                        ...
                        if (col.type === 'status' && col.options) {
                          return (
                            <td key={col.id} className="p-2.5 border-r border-[#171720]">
                              <select
                              ...
                              >
                                {col.options.map((opt) => (
                                  <option key={opt} value={opt} className="bg-[#0D0D12] text-[#F4F4F6]">{opt}</option>
                                ))}
                              </select>
                            </td>
                          );
      ...
      {activeSubTab === 'notes' && (
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <span className="text-xs text-[#707080]">Markdown enabled notes & strategy briefings</span>
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[9999] flex items-center justify-center p-4">
          <div className="bg-[#0D0D12] border border-[#22222B] rounded-2xl p-6 w-full max-w-sm space-y-4 shadow-2xl">
            <div className="flex items-center justify-between border-b border-[#1C1C26] pb-3">
              ...
              <button onClick={() => setShowAddCol(false)} className="text-[#606070] hover:text-white">
                <X className="w-4 h-4" />
              </button>
            </div>
            ...
            <form onSubmit={handleAddColumn} className="space-y-4">
              ...
              <div className="flex items-center justify-end gap-2 pt-2">
                <button type="button" onClick={() => setShowAddCol(false)} className="hesics-btn-ghost text-xs">
                  Cancel
                </button>
                <button type="submit" className="hesics-btn-primary text-xs px-4">
                  Add Property
                </button>
              </div>
            </form>
