...
import {
  ...
  exportMonthlyIncomeToExcel,
  exportMonthlyExpenseToExcel,
    ...
    try {
      ...
      if (format === "excel" || format === "both") {
        if (reportType === "income" || reportType === "combined") {
          exportMonthlyIncomeToExcel(
            filteredIncome,
            `HESICS_Income_${suffix}.xlsx`,
          );
        }
        if (reportType === "expense" || reportType === "combined") {
          exportMonthlyExpenseToExcel(
            filteredExpense,
            `HESICS_Expense_${suffix}.xlsx`,
          );
        }
          ...
          setTimeout(
            () =>
              exportFinanceToExcel(
                filteredIncome,
                filteredExpense,
                `HESICS_FinancialLedger_${suffix}.xlsx`,
              ),
            600,
          );
        }
      ...
      if (format === "pdf" || format === "both") {
        if (reportType === "income" || reportType === "combined") {
          ...
          a.href = url;
          a.download = `HESICS_Income_Report_${suffix}.pdf`;
        ...
        if (reportType === "expense" || reportType === "combined") {
          const doc2 = await generateExpenseReportPDF(
            filteredExpense,
            org,
            label,
          );
          ...
          a2.href = url2;
          a2.download = `HESICS_Expense_Report_${suffix}.pdf`;
    ...
    <div className="fixed inset-0 bg-black/85 backdrop-blur-md z-confirmDialog flex items-center justify-center p-4">
      <div className="bg-[#0D0D11] border border-[#22222B] rounded-3xl w-full max-w-xl shadow-2xl overflow-hidden">
        ...
        <div className="flex items-center justify-between px-6 py-5 border-b border-[#1C1C26]">
          <div>
            ...
            <p className="text-xs text-[#808090] mt-0.5">
              Customize date range, type and format before downloading.
            </p>
          </div>
          <button
            onClick={onClose}
            className="text-[#606070] hover:text-white p-1.5 rounded-xl hover:bg-[#16161D]"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        <div className="p-6 space-y-5">
              ...
              {(
                ...
                <button
                  ...
                  className={`flex flex-col items-center gap-1.5 p-3 rounded-xl border text-xs font-medium transition-all ${
                    reportType === v
                      ? "border-[#77727E]/60 bg-[#77727E]/10 text-[#F4F4F6]"
                      : "border-[#1E1E28] bg-[#09090C] text-[#707080] hover:border-[#2A2A38]"
                  }`}
                >
                  {v === "income" && (
                    <TrendingUp
                      className={`w-4 h-4 ${reportType === v ? "text-emerald-400" : ""}`}
                    />
                  )}
                  {v === "expense" && (
                    <TrendingDown
                      className={`w-4 h-4 ${reportType === v ? "text-rose-400" : ""}`}
                    />
                  )}
                  {v === "combined" && (
                    <FileText
                      className={`w-4 h-4 ${reportType === v ? "text-[#77727E]" : ""}`}
                    />
                  )}
                  {l}
                </button>
            ...
            <div className="grid grid-cols-3 gap-2">
              {(
                [
                  ...
                  ["excel", "Excel Ledger", "FileSpreadsheet"],
                  ["both", "Both", "Download"],
                ] as const
              ).map(([v, l]) => (
                <button
          ...
          <div className="grid grid-cols-3 gap-3 p-4 bg-[#09090C] border border-[#1A1A24] rounded-2xl">
            <div className="text-center">
              <div className="text-[10px] text-[#606070] uppercase tracking-wider">
                Income Entries
              </div>
              <div className="text-base font-bold text-emerald-400 mt-1">
                {filteredIncomes.length}
              </div>
              <div className="text-[9px] text-[#505060] font-mono">
            ...
            <div className="text-center">
              ...
              <div
                className={`text-base font-bold mt-1 ${totalInc - totalExp >= 0 ? "text-emerald-400" : "text-rose-400"}`}
              >
        ...
        <div className="px-6 py-4 border-t border-[#1A1A22] flex items-center justify-between">
          <button onClick={onClose} className="hesics-btn-ghost text-xs">
            Cancel
          </button>
          <button
