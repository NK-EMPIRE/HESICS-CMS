...
export const AppShell: React.FC<AppShellProps> = ({
  ...
  const [persistenceStatus, setPersistenceStatus] = useState(() => db.getPersistenceStatus());
  const location = useLocation();
  const navigate = useNavigate();
  const org = db.getOrg();

  React.useEffect(() => {
    return db.subscribe(() => setPersistenceStatus(db.getPersistenceStatus()));
  }, []);
  ...
  return (
    <div className="flex h-screen bg-[#050505] text-[#A1A1AA] font-sans overflow-hidden select-none">
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-hidden bg-[#08080B]">
        {persistenceStatus.state !== "synced" && (
          <div className={`mx-4 mt-3 rounded-xl border px-4 py-2 text-xs ${persistenceStatus.state === "error" ? "border-rose-900/50 bg-rose-950/20 text-rose-300" : persistenceStatus.state === "local" ? "border-amber-900/50 bg-amber-950/20 text-amber-300" : "border-sky-900/50 bg-sky-950/20 text-sky-300"}`} role="status">
            {persistenceStatus.state === "local" && "Local-only mode: Firebase is not configured, so changes will not persist across devices."}
            {persistenceStatus.state === "connecting" && "Connecting to the shared HESICS data store…"}
            {persistenceStatus.state === "error" && `Shared data synchronization failed${persistenceStatus.error ? `: ${persistenceStatus.error}` : "."}`}
          </div>
        )}
