...
export const AppShell: React.FC<AppShellProps> = ({
  ...
  const baseNavItems = [
    { path: '/dashboard', label: 'Dashboard', icon: LayoutDashboard, perm: 'deals:read' as PermissionKey },
    ...(isSuper
      ? [{ path: '/private-space', label: 'Private Vault', icon: Lock, perm: 'superadmin:vault' as PermissionKey, badge: 'Superadmin', badgeColor: 'bg-amber-950/40 text-amber-300 border-amber-800/50' }]
      : []),
    { path: '/clients', label: 'Clients', icon: Users, perm: 'clients:read' as PermissionKey, badge: coldCount > 0 ? `${coldCount} cold` : undefined, badgeColor: 'bg-amber-950/40 text-amber-400 border-amber-900/40' },
    { path: '/deals', label: 'Deals Board', icon: Kanban, perm: 'deals:read' as PermissionKey },
    { path: '/meetings', label: 'Meetings & Calendar', icon: Video, perm: 'clients:read' as PermissionKey },
    { path: '/finance', label: 'Finance & Tax', icon: DollarSign, perm: 'finance:read' as PermissionKey },
    { path: '/quotations', label: 'Quotations', icon: FileText, perm: 'invoices:read' as PermissionKey },
    { path: '/invoices', label: 'Invoices', icon: Receipt, perm: 'invoices:read' as PermissionKey },
    { path: '/agreements', label: 'Agreements', icon: FileSignature, perm: 'clients:read' as PermissionKey },
    { path: '/team', label: 'Team & RBAC', icon: ShieldCheck, perm: 'team:manage' as PermissionKey },
    ...(isAdmin
      ? [{ path: '/audit-logs', label: 'Audit Logs', icon: History, perm: 'team:manage' as PermissionKey }]
      : []),
    { path: '/settings', label: 'Settings', icon: Settings, perm: 'clients:read' as PermissionKey },
  ];
        ...
        <div className="p-2.5 border-t border-[#191920] bg-[#070709] space-y-2">
          ...
          >
            <div className="flex items-center gap-2 min-w-0">
              ...
              {!collapsed && <span className="truncate text-rose-300 font-semibold">Sign Out</span>}
            </div>
            {!collapsed && <span className="text-[10px] text-[#505060]">Exit</span>}
          </button>
              ...
              {!collapsed && (
                <div className="truncate min-w-0">
                  ...
                  <div className="text-[9px] text-[#707080] truncate mt-0.5">
                    {displayTierName(activeUser.hierarchy, activeUser.role_name)}
                  </div>
                </div>
              )}
            </div>
