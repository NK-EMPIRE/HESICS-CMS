...
export const ClientDetail: React.FC<ClientDetailProps> = ({
  ...
  const invoices = db
    ...
    .filter(
      (i) =>
        i.client_name?.toLowerCase() === client.name.toLowerCase() ||
        (i as any).client_id === client.id,
    );
  const quotations = db
    .getQuotations()
    .filter(
      (q) =>
        q.client_name?.toLowerCase() === client.name.toLowerCase() ||
        (q as any).client_id === client.id,
    );
  ...
  const handleQuickNote = (e: React.FormEvent) => {
    ...
    showToast(
      "Note Logged",
      "Communication note recorded on client timeline.",
      "success",
    );
  };
    ...
    <div className="space-y-6 w-full pb-16">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-[#808090] mt-0.5">
              {client.company_name || "Individual Entity"} · Client ID:{" "}
              <span className="font-mono text-[#D4D4D8]">{client.id}</span>
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
        <div className="hesics-card p-4 space-y-1">
          <div className="text-[10px] text-[#606070] uppercase font-mono tracking-wider">
            Total Billed
          </div>
          <div className="text-lg font-bold font-mono text-[#F4F4F6]">
        ...
        <div className="hesics-card p-4 space-y-1">
          ...
          <div
            className={`text-lg font-bold font-mono ${totalOutstanding > 0 ? "text-amber-400" : "text-[#707080]"}`}
          >
        ...
        <div className="hesics-card p-4 space-y-1">
          ...
          <div className="text-[10px] text-indigo-400/80">
            {agreements.filter((a) => a.status === "signed").length} executed
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center gap-1.5 border-b border-[#1A1A22] pb-2 overflow-x-auto">
        {(
          ...
          >
            <Icon
              className={`w-3.5 h-3.5 ${tab === tKey ? "text-[#77727E]" : ""}`}
            />
            <span>{label}</span>
          </button>
        ))}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="hesics-card p-6 space-y-5 lg:col-span-2">
            <h2 className="text-sm font-bold text-[#F4F4F6]">
              Corporate Profile & Contact Parameters
            </h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
              <div className="p-3 bg-[#0A0A0E] border border-[#1A1A22] rounded-xl space-y-1">
                <div className="text-[#606070] text-[10px] uppercase font-mono">
                  Primary Email
                </div>
                <div className="text-[#F4F4F6] font-medium">
                  {client.email || "—"}
                </div>
              </div>
              <div className="p-3 bg-[#0A0A0E] border border-[#1A1A22] rounded-xl space-y-1">
            ...
            <form
              ...
              className="pt-4 border-t border-[#1A1A22] space-y-2.5"
            >
              <label className="hesics-label">
                Append Real-Time Communication Note
              </label>
          ...
          <div className="flex items-center justify-between border-b border-[#1A1A22] pb-3.5">
            <div>
              ...
              <p className="text-xs text-[#707080]">
                Legally executed contracts under Indian IT Act 2000.
              </p>
            </div>
          </div>
                ...
                >
                  <div>
                    <div className="flex items-center gap-2">
                      <span className="font-mono text-xs font-semibold text-[#F4F4F6]">
                  ...
                  <div className="flex items-center gap-2">
                    {agr.status === "pending" && (
                      <button
                        onClick={() => {
                          ...
                          showToast(
                            "Sign Link Copied",
                            "Client e-signature URL copied to clipboard.",
                            "success",
                          );
                        }}
                        className="hesics-btn-secondary text-xs"
                    ...
                    <button
                      ...
                      className="hesics-btn-secondary text-xs"
                    >
                      <Download className="w-3.5 h-3.5 text-[#77727E]" />{" "}
                      Download PDF
                    </button>
                  </div>
                </div>
        ...
        <div className="hesics-card p-6 space-y-4">
          <div className="flex items-center justify-between border-b border-[#1A1A22] pb-3.5">
            <div>
              ...
              <p className="text-xs text-[#707080]">
                All tax invoices issued to {client.name}.
              </p>
            </div>
            <button
              onClick={() => setIsInvoiceModalOpen(true)}
              ...
              {invoices.map((inv) => (
                ...
                >
                  <div>
                    ...
                    <div className="text-[11px] text-[#707080] mt-0.5">
                      Due Date: {inv.due_date || "On Receipt"} · Total: ₹
        ...
        <div className="hesics-card p-6 space-y-4">
          <div className="flex items-center justify-between border-b border-[#1A1A22] pb-3.5">
            <div>
              ...
              <p className="text-xs text-[#707080]">
                Formal proposals submitted to {client.name}.
              </p>
            </div>
            <button
              onClick={() => setIsQuotationModalOpen(true)}
              ...
              {quotations.map((qt) => (
                ...
                >
                  <div>
                    <div className="flex items-center gap-2">
                      <span className="font-mono text-xs font-semibold text-[#F4F4F6]">
                        {qt.quotation_number || (qt as any).quote_number}
                      </span>
                    ...
                    <div className="text-[11px] text-[#707080] mt-0.5">
                      Valid Until:{" "}
                      {qt.valid_until || (qt as any).expiry_date || "30 Days"} ·
                      Amount: ₹{Number(qt.total || 0).toLocaleString("en-IN")}
                    </div>
                  </div>
        ...
        <div className="hesics-card p-6 space-y-4">
          <div className="flex items-center justify-between border-b border-[#1A1A22] pb-3.5">
            <div>
              ...
              <p className="text-xs text-[#707080]">
                Audit log of calls, meetings, DMs, and notes.
              </p>
            </div>
            <button
              onClick={() => setIsActivityModalOpen(true)}
                  ...
                  <div className="space-y-1">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs font-semibold text-[#F4F4F6]">
                        {(act as any).title || act.outcome || act.notes}
                      </span>
                    </div>
