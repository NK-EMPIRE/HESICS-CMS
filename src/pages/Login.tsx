...
import {
  ArrowRight, Mail, CheckCircle2, Loader2, AlertCircle,
  Lock, Sparkles, ArrowLeft, KeyRound
} from 'lucide-react';
...
export const Login: React.FC<LoginProps> = ({ onLogin }) => {
  ...
  const handleSetupPassword = async (e: React.FormEvent) => {
    ...
    if (!existingUser) {
      setLoading(false);
      setErrorMsg('This email is not invited to HESICS yet. Please contact your admin.');
      return;
    }
  ...
  const handleForgotPassword = async (e: React.FormEvent) => {
    ...
    setLoading(false);
    if (res.success) {
      setSuccessMsg('Password reset link sent! Check your inbox to choose a new password.');
    } else {
  ...
  const handleMagicLink = async (e: React.FormEvent) => {
    ...
    setLoading(false);
    if (res.success) {
      setSuccessMsg('Sign-in link sent! Click the link in your email to authenticate.');
    } else {
        ...
        <div className="bg-[#0E0E13] border border-[#1C1C24] rounded-2xl p-7 shadow-2xl space-y-6">
          ...
          <div className="flex flex-col items-center text-center space-y-2.5">
            ...
            <div>
              <h1 className="text-base font-bold text-[#F4F4F6] tracking-tight">
                {authView === 'setup_password' ? 'Set Up Your Password' : 'Sign in to HESICS'}
              </h1>
              <p className="text-xs text-[#808090] mt-0.5">
                {authView === 'setup_password' ? 'Create a secure password for your invited account' : 'Make It Simple.'}
              </p>
            </div>
          </div>
            ...
            <form onSubmit={handleSignIn} className="space-y-3.5">
              ...
              <div>
                <div className="flex items-center justify-between mb-1">
                  ...
                  <button
                    type="button"
                    onClick={() => { clearMessages(); setAuthView('forgot_password'); }}
              ...
              <div className="relative my-3 flex items-center justify-center">
                <div className="absolute inset-0 flex items-center"><div className="w-full border-t border-[#1C1C26]" /></div>
                <span className="relative bg-[#0E0E13] px-2 text-[10px] uppercase font-mono text-[#505060]">or</span>
              </div>
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <button
                  type="button"
                  onClick={() => { clearMessages(); setAuthView('magic_link'); }}
          ...
          {authView === 'setup_password' && (
            <form onSubmit={handleSetupPassword} className="space-y-3.5">
              ...
              >
                {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : 'Set Password & Enter HESICS'}
              </button>

              <button
                type="button"
                onClick={() => { clearMessages(); setAuthView('signin'); }}
