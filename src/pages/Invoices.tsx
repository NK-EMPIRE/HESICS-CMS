...
const INVOICE_STATUS_OPTIONS: Option[] = [
  ...
  },
  {
    ...
    label: "Paid",
    badge: "Paid",
    badgeColor: statusBadge["paid"],
  },
  {
    value: "overdue",
    label: "Overdue",
    badge: "Overdue",
    badgeColor: statusBadge["overdue"],
  },
  {
    value: "cancelled",
    label: "Cancelled",
    badge: "Void",
    badgeColor: statusBadge["cancelled"],
  },
];
...
export const Invoices: React.FC<InvoicesProps> = ({ activeUser }) => {
  ...
  const [previewingInvoice, setPreviewingInvoice] = useState<Invoice | null>(
    null,
  );
  ...
  const handleQuickStatusChange = (inv: Invoice, newStatus: InvoiceStatus) => {
    ...
    showToast(
      "Status Updated",
      `Invoice #${inv.invoice_number} set to ${newStatus.toUpperCase()}`,
    );
  };
  ...
  const handleDownloadPDF = async (inv: Invoice) => {
    const doc = await generateInvoicePDF(
      inv,
      org,
      (inv.template_id as TemplateType) || "titanium",
    );
    ...
    if (config.dateMode === "month") {
      ...
      if (config.startDate)
        filtered = filtered.filter(
          (i) =>
            (i.issue_date || i.created_at).split("T")[0] >= config.startDate,
        );
      if (config.endDate)
        filtered = filtered.filter(
          (i) => (i.issue_date || i.created_at).split("T")[0] <= config.endDate,
        );
    }
                    ...
                    {canWrite ? (
                      <div className="w-28">
                        <CustomSelect
                          value={inv.status}
                          onChange={(v) =>
                            handleQuickStatusChange(inv, v as InvoiceStatus)
                          }
                      ...
                      <span
                        className={`inline-block text-[9px] font-bold uppercase px-2 py-0.5 rounded-md border ${statusBadge[inv.status]}`}
                      >
                        {inv.status}
                      </span>
                    )}
