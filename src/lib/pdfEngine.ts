...
export type TemplateType =
  "titanium" | "executive" | "corporate" | "commercial";
...
export const AVAILABLE_TEMPLATES: {
...
}[] = [
  {
    ...
    description: "Deep slate with #77727E metallic accents & clean grid",
    badge: "Titanium",
  },
  {
    id: "executive",
    name: "Executive Minimalist",
    description: "Ultra-clean high-ticket monochrome with accent bar",
    badge: "Executive",
  },
  {
    id: "corporate",
    name: "Corporate Enterprise",
    description: "Formal navy-charcoal structure with itemized lines",
    badge: "Enterprise",
  },
  {
    id: "commercial",
    name: "Classic Commercial",
    description: "Clean bordered compliance box with dual-tone header",
    badge: "Commercial",
  },
];
...
// ─────────────────────────────────────────────────────────────────────────────
function renderTitaniumPDF(
  doc: jsPDF,
  type: "TAX INVOICE" | "QUOTATION",
  docNumber: string,
  org: Organization,
  client: { name: string; email?: string; address?: string; gstin?: string },
  meta: { issueDate: string; dueDateOrValid: string; status: string },
  items: any[],
  subtotal: number,
  tax: number,
  total: number,
  ...
  doc.text(
    type === "TAX INVOICE" ? "BILLED TO:" : "PREPARED FOR:",
    110,
    startY,
  );
  ...
  doc.text(org.name || "HESICS", 16, yL);
  yL += 5;
  doc.setFont("helvetica", "normal");
  doc.setFontSize(8);
  doc.setTextColor(80, 80, 90);
  if (org.email) {
    doc.text(`Email: ${org.email}`, 16, yL);
    yL += 4.5;
  }
  if (org.address) {
    doc.text(org.address, 16, yL);
    yL += 4.5;
  }
  if (isTax && org.gstin) {
    doc.text(`GSTIN: ${org.gstin}`, 16, yL);
    yL += 4.5;
  }
  ...
  doc.setFontSize(9.5);
  doc.setTextColor(30, 30, 35);
  doc.text(client.name || "Client", 110, yR);
  yR += 5;
  doc.setFont("helvetica", "normal");
  doc.setFontSize(8);
  doc.setTextColor(80, 80, 90);
  if (client.email) {
    doc.text(`Email: ${client.email}`, 110, yR);
    yR += 4.5;
  }
  doc.text(`Issue Date: ${meta.issueDate}`, 110, yR);
  yR += 4.5;
  doc.text(
    `${type === "TAX INVOICE" ? "Due Date" : "Valid Until"}: ${meta.dueDateOrValid}`,
    110,
    yR,
  );
  yR += 4.5;
  doc.text(`Status: ${meta.status.toUpperCase()}`, 110, yR);
  yR += 4.5;
  ...
  doc.text(`INR ${subtotal.toLocaleString("en-IN")}`, x + 71, afterY + 14, {
    align: "right",
  });
  let ty = afterY + 14;
  if (isTax) {
    ...
    doc.text(`INR ${tax.toLocaleString("en-IN")}`, x + 71, ty, {
      align: "right",
    });
  }
  ty += 7;
  ...
  doc.text(`INR ${total.toLocaleString("en-IN")}`, x + 71, ty, {
    align: "right",
  });
  ...
  doc.setTextColor(110, 110, 120);
  [
    "1. Remit via NEFT / RTGS / Direct Bank Transfer.",
    "2. Reference document number on all remittance receipts.",
    "3. Generated by HESICS Business Operating System.",
  ].forEach((t, i) => doc.text(t, 16, afterY + 20 + i * 4.5));
  ...
  doc.text(
    "HESICS Titanium Edition  •  Make It Simple  •  hesics1@gmail.com  •  hub-hesics.vercel.app",
    105,
    284,
    { align: "center" },
  );
  doc.text("Confidential & Privileged Commercial Document", 105, 289, {
    align: "center",
  });
}
...
function renderExecutivePDF(
  ...
  doc.setFontSize(10);
  doc.setTextColor(20, 20, 24);
  doc.text(client.name || "Client", 14, yL);
  yL += 5;
  doc.setFont("helvetica", "normal");
  doc.setFontSize(8);
  doc.setTextColor(90, 90, 100);
  if (client.email) {
    doc.text(`Email: ${client.email}`, 14, yL);
    yL += 4.5;
  }
  if (org.address) {
    doc.text(`Location: ${org.address}`, 14, yL);
    yL += 4.5;
  }
  ...
  doc.setFontSize(8);
  doc.setTextColor(90, 90, 100);
  doc.text(`Date Issued: ${meta.issueDate}`, 120, yR);
  yR += 4.5;
  doc.text(
    `${type === "TAX INVOICE" ? "Payment Due" : "Valid Until"}: ${meta.dueDateOrValid}`,
    120,
    yR,
  );
  yR += 4.5;
  doc.text(`Status: ${meta.status.toUpperCase()}`, 120, yR);
  yR += 4.5;
  ...
  doc.text(`₹${subtotal.toLocaleString("en-IN")}`, 196, afterY + 10, {
    align: "right",
  });
  let ty = afterY + 10;
  if (isTax) {
  ...
  doc.text(`₹${total.toLocaleString("en-IN")}`, 196, ty + 3, {
    align: "right",
  });
  ...
  doc.text(
    "HESICS Executive Suite  •  hesics1@gmail.com  •  hub-hesics.vercel.app",
    105,
    286,
    { align: "center" },
  );
...
function renderCorporatePDF(
  ...
  doc.text(
    `Issued: ${meta.issueDate}  |  Terms: ${meta.dueDateOrValid}`,
    108,
    startY + 20,
  );
  ...
  doc.text(`INR ${subtotal.toLocaleString("en-IN")}`, x + 73, afterY + 14, {
    align: "right",
  });
  let ty = afterY + 14;
  if (isTax) {
    ty += 7;
    doc.text("GST (18%):", x + 5, ty);
    doc.text(`INR ${tax.toLocaleString("en-IN")}`, x + 73, ty, {
      align: "right",
    });
  }
  ty += 7;
  ...
  doc.text(`INR ${total.toLocaleString("en-IN")}`, x + 73, ty, {
    align: "right",
  });
  ...
  doc.text(
    "HESICS Corporate Enterprise Division  •  Official Accounting Record  •  hub-hesics.vercel.app",
    105,
    285,
    { align: "center" },
  );
...
function renderCommercialPDF(
  ...
  doc.text(`INR ${subtotal.toLocaleString("en-IN")}`, x + 71, afterY + 13, {
    align: "right",
  });
  let ty = afterY + 13;
  if (isTax) {
  ...
  doc.setTextColor(120, 120, 130);
  doc.text(
    "Legally binding commercial instrument governed under Indian IT Act 2000.",
    105,
    278,
    { align: "center" },
  );
  doc.text("HESICS Commercial Division  •  Make It Simple", 105, 283, {
    align: "center",
  });
}
...
export async function generateInvoicePDF(
  invoice: Invoice,
  org: Organization,
  template: TemplateType = "titanium",
): Promise<jsPDF> {
  ...
  const selectedTemplate = (template ||
    (invoice as any).template_id ||
    org.default_invoice_template ||
    "titanium") as TemplateType;
...
export async function generateQuotationPDF(
  ...
  const selectedTemplate = (template ||
    (quote as any).template_id ||
    org.default_quotation_template ||
    "titanium") as TemplateType;
  ...
  const meta = {
    ...
    dueDateOrValid:
      quote.valid_until || (quote as any).expiry_date || "30 Days",
  ...
  if (selectedTemplate === "executive") {
    renderExecutivePDF(
      doc,
      "QUOTATION",
      quote.quotation_number || (quote as any).quote_number || "QT-001",
      org,
      clientInfo,
      meta,
      items,
...
export async function generateIncomeReportPDF(
  ...
  autoTable(doc, {
    startY: 50,
    head: [
      ["#", "Client / Source", "Category", "Method", "Date", "Amount (INR)"],
    ],
    body: entries.map((e, i) => [
      i + 1,
      e.client_name || e.source_type || "-",
      e.category || "-",
      e.payment_method || "-",
      e.received_at || "-",
...
export async function generateExpenseReportPDF(
  ...
  autoTable(doc, {
    ...
    body: entries.map((e, i) => [
      ...
      e.vendor || e.category || "-",
      e.category || "-",
      e.gst_paid ? `INR ${Number(e.gst_paid).toLocaleString("en-IN")}` : "-",
      e.spent_at || e.date || "-",
      Number(e.amount || 0).toLocaleString("en-IN"),
    ]),
    theme: "grid",
...
export async function generateFinanceReportPDF(
  ...
  autoTable(doc, {
    ...
    body: incomes
      ...
      .map((e, i) => [
        i + 1,
        e.client_name || e.source_type || "Direct",
        e.category || "Service",
        e.received_at || e.created_at?.split("T")[0] || "-",
        Number(e.amount || 0).toLocaleString("en-IN"),
      ]),
    theme: "grid",
    headStyles: {
