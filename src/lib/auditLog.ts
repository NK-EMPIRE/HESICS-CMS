...
function getCategoryForAction(action: AuditAction): AuditLogEntry["category"] {
  ...
  if (action.startsWith("invoice.") || action.startsWith("quotation."))
    return "Billing";
  if (action.startsWith("income.") || action.startsWith("expense."))
    return "Finance";
  if (action.startsWith("user.") || action.startsWith("role.")) return "Team";
  if (action.startsWith("auth.") || action.startsWith("vault."))
    return "Security";
  return "Settings";
}
